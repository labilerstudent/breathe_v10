...
{ x+=44;
  ...
  noFill();  fill(360);text(slider[Slider_ID].name,x+w+10,y);
  ...
  check_box ( x-24 ,y,slider[Slider_ID].sinus  ,Slider_ID, 2 );
  check_box ( x-10 ,y,slider[Slider_ID].sinus2 ,Slider_ID, 3 );
}
...
{ boolean sinus=false,sinus2=false,beat=false,beat_negative=false, has_boundaries=false; 
  ...
  void self_correct ()
  { if((this.boundary_b-this.boundary_a)<0.03){this.boundary_a -=0.03 ; this.boundary_b +=0.03 ; }
  ...
  { if(this.sinus )this.value=(this.value+((sin( angle3/2 )+1)*abs(this.boundary_a-this.boundary_b))/2+min(this.boundary_a,this.boundary_b) )/2;  
    ...
    // Hier kommt 2 sinus variable hun
